/**
 * Simplified Wood Harvesting Test - No Pathfinder Dependency
 */
...
console.log('\n' + '='.repeat(60));
console.log('🧪 SIMPLIFIED WOOD HARVESTING TEST');
console.log('='.repeat(60) + '\n');

const bot = mineflayer.createBot({
  host: 'localhost',
  port: 25565,
  username: 'SimpleWoodTestBot',
  version: false
});
...
bot.on('spawn', async () => {
  ...
  try {
    ...
    // TEST: Run actual harvest with basic fallback
    console.log('\n🪵 TEST: Running Harvest (1 log only)');
    console.log('   Starting harvestWood(bot, 5, 1)... (small radius to avoid pathfinder)');

    try {
      const result = await harvestWood(bot, 5, 1); // Very small radius
